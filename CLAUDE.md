...
### Repo layout by game (`{game}` namespacing)

Each corpus is namespaced: `raw/{game}/`, `data/{game}/`, `schema/{game}_schema.md`,
`docs/{game}/`, and `tools/{game}_*.html`. `gs1` is the prototype; a new corpus
...
- **`scripts/` (top level)** — the cross-cutting / active rebuild chain:
  ...
  a second corpus actually needs them.
- **`scripts/gs1/`** — applied one-shot scripts (the `*_supplement.py` + `*_apply.py`
  data-merge pairs, superseded `build_planner.py`). Kept for provenance; their
  output is already baked into `data/gs1/`. Not part of any live rerun.
...
a time. See `docs/gs1/gs1_walkthrough_extraction_plan.md` for the batch strategy
and progress. `_batch_*.md` files in `data/gs1/` are intermediate batch artifacts.

### The HTML explorers

`tools/*.html` are standalone single-file apps. The active gs1 app is
`tools/gs1_codex.html` (the unified "Adept's Codex" — graph wiki + build planner),
regenerated by `scripts/build_codex.py`. Superseded single-purpose explorers live
in `tools/archive/` (class/equipment explorers + the old build planner the codex
replaced). Data is **embedded** in `<script type="application/json" id="data-...">`
blocks and read via `JSON.parse(...)` — they do not `fetch` the JSON files.
Consequence: after regenerating a `data/{game}/*.json`, rerun the generator (or
re-paste the embedded block by hand); the tool will not pick up changes
automatically.
