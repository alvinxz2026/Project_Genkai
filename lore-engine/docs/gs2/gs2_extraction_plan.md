...
| **characters** | walkthrough character index（darthmarth/darkslime）；`ultimalink`（8 角色职业） | 剧本源（story，仅背景，不提数值） | gs2 队伍：Felix/Jenna/Sheba/Piers + 回归的 Isaac/Garet/Ivan/Mia（详见 §3）。 |
