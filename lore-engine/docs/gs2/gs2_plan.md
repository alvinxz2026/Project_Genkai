...
| 6. 应用层 | 方向 + 分阶段见 [`gs2_app_brainstorm.md`](gs2_app_brainstorm.md) | 🔄 **Phase 1 MVP（连接层）✅** + **Build Planner ✅**：`build_codex_gs2.py`→`tools/gs2_codex.html`（wiki + 双向交叉链接 + hover tooltip + source inspector + planner tab）。**Planner**：移植 gs1 机制（Set-Djinn 分布→职业/stat/Psynergy），gs2 适配 = telago 区间 + ultimalink 定点行 OR 合并、`other`=每个非本命元素（Trickster 系）、native fallback；**8 adept 共享一个 72-Djinn 池（18/元素）**；smoke 5/5 pass。**三交付物各有第一版 ✅（2026-06-20）**：①Codex + Build Planner（17e79e6）/ ③中文 walkthrough（66b9969）/ ②**NotebookLM 投喂包**（`scripts/export_notebooklm_gs2.py`→`export/gs2_notebooklm/` 14 prose md，每实体 deduped/conflict-aware + 合并 en/zh walkthrough，13 source）。**▶ Phase 2 进度感（2026-06-21）**：① **Progression Seam ✅**（planner region 下拉门控 72-Djinn 池；同日修 GS1-djinn timing bug=null-area 锚到 Contigo 重逢点）；② **B2 access-gate 数据 ✅**（`access_gates_gs2.py`→`data/gs2/access_gates.json`，21 区 53 gated，GS2 无永久 missable→载 `return_required` 延迟门控）；③ **B2 Collection Tracker tab ✅**（codex 第三 tab：按 region 折叠清单 + localStorage 持久 + return-trip 徽章 + 「Reachable now」面板 + 与 planner 共享进度锚；Playwright 全检查通过，890KB）。**B1 攻略阅读器（spoiler 门控）✅（2026-06-21）**：codex 第 4 个 Guide tab，左 62-region 目录 + 右双语正文（中文为主 +zh/en 切换），正文实体名 auto-linkify 回 wiki（908 候选名、最长匹配 + 停用词表 + 每区首现去重），spoiler 门控复用共享进度锚（gated region 折叠成 reveal 遮罩）。**C3 锻造规划 + linkify 调优 ✅（2026-06-21）**：第 5 个 Forging tab（14 材料卡＝9 稀有＋5 rusty，材料中心反查＝forged_from 取逆，每卡「材料→哪里掉/找/买 + 锻成 N 选 1 产物」）；linkify 停用词加 Jupiter/Mercury/Master/Spirit/Break（链接 1667→1575，元素-召唤撞名误链清掉）。**下一步候选 = D1 boss 备战卡 / D2 图鉴 / tracker 小优化**。|
...
| 日期 | 进展 |
|---|---|
| 2026-06-21 | **C3 锻造规划（Forging tab）+ linkify 停用词调优 ✅**（`build_codex_gs2.py`，codex regen）。**C3**：第 5 个 tab，材料中心反查锻造（wiki 只给装备→forged_from，这里取逆＝材料→产物池）。`FORGE`＝从 `equipment.forged_from` 倒排出 14 材料（9 稀有材料 item + 5 rusty 装备），每张卡＝材料 chip + 「哪里弄到」（`rev.gearToMonsters`/`gearToShops`/`locFor` 三索引复用＝Dropped by/Found in/Sold at）+ 「Forges into one of N」产物 chip（hover 看数值、点击进 wiki）；分 Rare Materials/Rusty Equipment 两组。**数据квирk**：forged_from 用「Rusty Long Sword/Light Blade」但装备里都叫「Rusty Sword」（×4 同名不同 id）→这 2 个降级纯文本 + rusty-note（记 backlog，非 C3 修）。**linkify 调优**：数据驱动（Python 复刻 JS matcher 跑遍 62×2 区统计被链名频率）发现 Jupiter(42)/Mercury(36) 误链 summons（实为元素/灯塔/Djinn 类型，四基础召唤同名）→停用词加 Jupiter/Mercury/Master(类)/Spirit(怪)/Break(动词)，链接实例 1667→1575；多词名（Jupiter Lighthouse/Mars Star）+ 工具 Psynergy（Hover/Reveal/Pound）保留。Playwright：forge 14 卡 2 组 / Orihalcon「1 of 7」/ 材料点击落 wiki / Jupiter Lighthouse 区 bare jupiter·mercury 不再链 / 0 console error + 截图眼检。**下一步候选**：D1 boss 备战卡 / D2 图鉴 / tracker 小优化。 |
| 2026-06-21 | **Phase 2 第三刀：B1 攻略阅读器（Guide tab）✅**（`build_codex_gs2.py`，codex regen 1.21MB）。新增第 4 个 tab 把 `walkthrough_zh/`+`walkthrough/`（en）62 节点变成可读产品。**数据**：`load_walkthrough()` 解析 frontmatter（region_id/region/order/kind）+ 双语 body，按 region_id join、order 排序，嵌 `__WT__`。**UI**：左 62-region 目录（order#+kind 徽章）+ 右正文阅读器；JS 轻量 markdown 渲染器（标题/有序无序列表/bold/italic/段落）。**实体名 auto-linkify**：从 DB 建 908 候选名（characters/locations/bosses/summons 全收 + 多词名 + 长度≥5 非停用词），停用词表挡掉 ~60 个与英文常用词撞名的单词名（Pole/Frost/Force/Echo…），正则 longest-first + ASCII 词界 lookaround（中文相邻可匹配）+ **每区首现去重**，渲染成 `.ref` chip 白嫖现有 hover tooltip + 点击跳 wiki。**语言**：中文为主 + zh/en 切换钮（用户拍板）。**spoiler 门控**：复用共享进度锚（`fillRegionSelect`+`onProgressChange`），order>gate 的 region 在目录灰显🔒、正文显 reveal 遮罩，`wtRevealed` 单区解锁，默认 sandbox。Playwright 全检查：62 TOC 行 / 29 段 6 标题 6 列表 / 23 链接 0 重复 / zh-en 切换 / gate=5→56 锁 + reveal / 跨 tab 同步（planner 3-72、tracker）/ ref 点击落 wiki detail / 0 console error。**下一步候选**：D1 boss 备战卡 / D2 图鉴 / C3 锻造 / linkify 停用词调优。 |
| 2026-06-21 | **Phase 2 第二刀：B2 Collection Tracker tab ✅ + Seam timing bug 修复 ✅**（均在 `build_codex_gs2.py`，codex regen 890KB）。**① Seam bug（用户发现）**：早期进度门控误显 28 个 GS1-transferred djinn（`area==null` 默认 order=-1=开局可用）；实则它们随 Isaac 队伍在 Contigo 重逢才到 → `djinnRegionOrder` 对 null-area 返回 `REGION_ORDER["contigo"]`(49)。验证逐点对账 Python：region0 池 28→**0**、Daila(3) 29→**1**(仅 Echo)、Contigo(49) 64、full sandbox 仍 72。**② B2 Tracker tab**：新增第三 tab，消费 `access_gates.json`+`locations.json`。功能=按 region 折叠清单（57 区有 collectible）+ 复选框 localStorage 持久（`gs2-codex-collection`）+ **return-trip 徽章**（门未开显 `📍 needs Frost`、开了显 `✓ reachable`）+ **「Reachable now, not yet collected」面板**（你已具备前置 Psynergy/事件但还没勾的 gated 物，= 开放世界版「该回去拿」提醒，非「永久错过」）+ collectible 名链入 Wiki。**共享进度锚**：planner 与 tracker 共用 `progressIndex`，任一 region 下拉变更经 `setProgress()` 同步两个 selector + 重渲两 tab（planner 池门控 + tracker reachable）。「ready now」判定=`PSY_ACQUIRED_ORDER`（8 个 derive 自 locations.psynergy_here + 6 个 utility/class psy[Lift/Hover/Carry/Force/Growth/Cyclone] curated）+ `EVENT_ORDER` 事件锚（近似 region order，未知事件保守判未开）。Playwright 全 6 检查 + seam fix + badge 双模式 + 持久化 + 跨 tab 同步全过、0 console error。**下一步候选**：B1 walkthrough 阅读器（spoiler 门控）/ D1 boss 备战卡 / tracker 小优化（区级展开记忆、event 锚精修）。 |
| 2026-06-21 | **Phase 2 起步：① Progression Seam ✅ + ② B2 access-gate 数据 ✅**。**①** 激活 codex Build Planner 的进度门控：`build_codex_gs2.py` 实装 `djinnRegionOrder(d)`（djinn.location.area → locations.order join，null-area=起始 djinn 恒可用、多 area 取 min）+ 「Progress gate」region 下拉（62 项）+ `X/72 Djinn available` hint，换掉原 PROGRESSION SEAM 占位。Playwright 验证：full=72、Daila=29、Kibombo=44、Mars LH=70（Python 逐点对账一致）；+按钮按门控池封顶、over-budget 显红不静默 clamp、跨 tab 持久、0 console error。codex regen 865KB。**②** B2 missable/access 层：新建 curated sidecar `data/gs2/access_gates.json`（builder `access_gates_gs2.py`，验证每 region_id + item 名都能 resolve 到该区 collectible，否则 FATAL）→ **21 区 53 个 gated collectible**（return_required 51 / first_visit_only 1 / transfer_only 1=Shine 需 GS1 Force Orb）；top 门控 Psynergy=Frost 14·Sand 12·Lift 8·Hover 8。**关键结论（非缺口）**：GS2 开放世界（有船后全可回头）= **无永久 missable**，故 `missable` 全 false、`point_of_no_return` 全 false；B2「离场提醒」真正吃的是 `return_required`（带哪个 Psynergy/事件回来拿）。门控判定靠 prose 显式延迟语（"return later with Frost"/"* reachable after Piers"），非按 Psynergy 时间线推断。**下一步**：把 access_gates 接进 codex（B2 collection tracker + localStorage + 离场提醒 UI），用 ① 已建的 region 选择器作进度锚。 |
