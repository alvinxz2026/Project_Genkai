...
def process():
    directory = "raw/gs2/_chapters/darthmarth"
    for filename in sorted(os.listdir(directory)):
        ...
        if filename == "00-front.md" or is_header_only:
            ...
            covers = []
            region = ""
        elif len(toc) == 1 and toc in "abcdefghijklmnopqrstuvwxy":
            # Walkthrough areas
            ...
            region = title.split(" -")[0].strip() # remove suffix like -Bginn- if present
        elif toc == "5": # Boss Guide
            kind = "prose-walkthrough"
            covers = ["bosses"]
            region = ""
        elif toc == "6": # Djinn Guide
            kind = "data-table"
            covers = ["djinn"]
            region = ""
        elif toc == "7": # Summons FAQ
            kind = "data-table"
            covers = ["summons"]
            region = ""
        elif toc == "9": # Forged Items Guide
            kind = "data-table"
            covers = ["forging"]
            region = ""
        elif toc == "10": # General Item Guide
            kind = "data-table"
            covers = ["items"]
            region = ""
        elif toc == "11": # Class Guide
            kind = "data-table"
            covers = ["classes"]
            region = ""
        elif toc == "12": # Psynergy Guide
            kind = "data-table"
            covers = ["psynergy"]
            region = ""
        elif toc in ("1", "2", "3", "8", "13", "14"):
            kind = "meta"
            covers = []
            region = ""
        else:
            if "walkthrough" in title.lower() and not is_header_only:
                kind = "prose-walkthrough"
