...
def process():
    directory = "raw/gs2/_chapters/ikillkenny"
    for filename in sorted(os.listdir(directory)):
        ...
        if filename == "00-front.md" or is_header_only:
            ...
            covers = []
            region = ""
        elif toc.startswith("chapter-4 > ") or toc == "chapter-4":
            # Walkthrough areas
            ...
            region = title
        elif toc.startswith("chapter-5 > ") or toc == "chapter-5": # Boss Guide
            kind = "prose-walkthrough"
            covers = ["bosses"]
            region = ""
        elif any(toc.startswith(f"chapter-{x}") for x in ["1", "2", "3", "6"]):
            kind = "meta"
            covers = []
            region = ""
        else:
