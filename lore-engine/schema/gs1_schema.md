...
- [Schema: `locations`](#schema-locations)
  - [Type Enum](#location-type-enum)
  - [Example Entry: Mercury Lighthouse](#example-entry-mercury-lighthouse)
  - [Derived view: `location_refs.json`](#derived-view-location_refsjson)
  - [Notes for CC (Extraction Instructions)](#notes-for-cc-extraction-instructions-9)
- [Schema: `characters`](#schema-characters)
  - [Example Entry: Isaac](#example-entry-isaac)
...
| `gs1-walkthrough` | Cleaned distilled walkthrough `data/gs1/gs1_walkthrough.md` (synthesis of telago/electrospecter/bfgamer/ikillkenny). Used by `locations` for the canonical place list, chapter ordering, and one-line summaries. |
| `gs1-maps` | Cleaned ASCII-map compendium `data/gs1/gs1_maps.md` (Shotgunnova/ElectroSpecter/Telago/BFGamer). Used by `locations` for `type` categorization and `has_map`. |

Add new rows here when new sources are ingested.

### Cross-Entity Links (foreign keys)

Entities reference each other. Most references are stored as the human-readable
`name`/`character` string, with a resolved foreign-key id added **next to it** by
`scripts/links_normalize.py` (the name stays for readability; the id is the link
of record). The id fields are generated, never hand-authored — rerun the script
after editing any entity, then `scripts/links_audit.py` to verify.

| Reference site | id field | -> target |
|---|---|---|
| `classes.psynergy[]` | `id` | `psynergy` |
| `classes.available_to[]` | `character_id` | `characters` |
| `classes.available_to[].djinn_requirements[]` | `parsed[]` | structured `{element,min,max}` |
| `shops.stock[]` | `ref_type` + `ref_id` | `equipment` \| `items` |
| `monsters.drops.items[]` | `ref_type` + `ref_id` | `equipment` \| `items` |
| `monsters` | `djinn_id` / `is_djinn_enemy` | `djinn` |
| `psynergy.acquired_via_item` | `item_id` | `items` |

Character references in `equipment.equippable_by` and `psynergy.available_to` stay
as bare names (natural keys); `links_audit.py` validates them against `characters`.
`locations` uses a separate derived view (`location_refs.json`) instead of inline ids.
...
available_to            array of object   YES        One object per character who can take this
  ...
  [].character_id       string            YES        FK -> characters.json id (e.g. "isaac").
                                                     Generated by scripts/links_normalize.py.
  [].djinn_requirements array of object   YES        Djinn setups per source. Sources express
                                                     ...
                                                     The `terence` source is AUTHORITATIVE for
                                                     matching: see note below.
    [].requirement      string            YES        Verbatim-ish requirement prose.
                                                     e.g. "0-1 Earth Djinn", "3 Mars, 4 Jupiter".
                                                     For terence rows: "Earth 0-2, Fire 5, Wind
                                                     0-2, Water 0-2" (all four elements).
    [].parsed           array of object   YES        Structured form of `requirement`. For non-
                                                     terence sources, generated by
                                                     scripts/links_normalize.py (element names
                                                     normalized Venus->earth, Mars->fire, etc; lists
                                                     only the named elements). [] when the prose has
                                                     no element-count (e.g. Jenna's fixed class
                                                     note). For terence rows, generated by
                                                     scripts/build_terence_class_reqs.py and lists
                                                     ALL FOUR elements (links_normalize preserves
                                                     these verbatim).
      [].element        string            YES        "earth" | "fire" | "wind" | "water"
      [].min            integer           YES        Min djinn of that element ("0-1" -> 0).
      [].max            integer           YES        Max djinn of that element ("0-1" -> 1).
...
psynergy                array of object   YES        Psynergy learnable in this class. Union of
  ...
  [].id                 string            YES        FK -> psynergy.json id. Generated by
                                                     scripts/links_normalize.py. "Blast" (two
                                                     distinct series sharing the display name) is
                                                     disambiguated by the class's other psynergy.
...
### Djinn-to-class matching (`terence` source)

In-game class assignment depends on the Djinn count of **all four elements at
once**, not a single off-element. The other sources (`plz2bstfu-class`,
`strawhat`, `aku-chi`) record only one off-element range per class and so cannot
resolve mixes that touch a second off-element. The `terence` source — extracted
from `raw/gs1/Djinn Class Mechanics FAQ - Terence.txt` by
`scripts/build_terence_class_reqs.py` — encodes the complete table and is the
authoritative source for the Build Planner matcher. Conventions:

- Each `source:"terence"` requirement is **one alternative row** and lists all
  four elements. A class has 1+ such rows; a Djinn mix matches the class iff
  **some** row has every element count within its `[min,max]` (rows are OR-combined).
- `min == max == 0` encodes the table's `---` (exactly zero — this constrains the
  base element too in dual classes, e.g. Mia's White Mage needs 0 Mercury).
- Disjoint counts in the source (e.g. Fire `5,7`, which excludes 6) are split
  across separate rows.
- Among several matching classes, the planner shows the **most specific** (tightest
  total range) as primary and the rest as "also valid".
...
acquired_via_item       object | null     YES        For psynergy granted by equipping an item.
  ...
  .item_id              string            YES        FK -> items.json id (e.g. "catch-beads").
                                                     Generated by scripts/links_normalize.py.
...
stock               array of object   YES        Everything the town sells.
  ...
  [].ref_type       string            YES        "equipment" | "item" — which file ref_id points
                                                 to. Generated by scripts/links_normalize.py.
  [].ref_id         string            YES        FK -> equipment.json or items.json id.
  [].category       string            YES        One of: "weapon" | "armor" | "item"
...
A lightweight **gazetteer / hub index** — one entry per GS1 place. This file is
deliberately NOT a structured map: ASCII maps live in `data/gs1/gs1_maps.md` and
walkthrough prose in `data/gs1/gs1_walkthrough.md`. The JSON exists to (1) give a
canonical, deduplicated list of place names with type + chapter + a one-line
summary, and (2) act as the hub that the location strings scattered across the
other JSONs (`djinn.location.area`, `equipment.acquisition.location`,
`monsters.found[]`, `bosses.location`, shops, `items.acquisition.location`)
reverse-index onto.

File: `data/gs1/locations.json`

```
Field               Type                Required   Notes
---------------------------------------------------------------------------
id                  string              YES        Lowercase, hyphens. e.g. "mercury-lighthouse".
name                string              YES        In-game place name. e.g. "Mercury Lighthouse".
game                string              YES        Always "gs1".
type                string              YES        One of the Location Type Enum below.
region              string | null       YES        Coarse world region: "Angara" | "Gondowan" |
                                                   "Sea". null for the world map / unknown.
chapter_first_seen  string | null       YES        Walkthrough chapter of first arrival, using
                                                   gs1_walkthrough.md numbering. e.g. "Chapter 3",
                                                   "Prologue", "Epilogue". null for the world map.
summary             string              YES        One factual sentence from the walkthrough
                                                   narrative. Do not invent lore.
has_map             boolean             YES        true if gs1_maps.md has an ASCII map for it.
aliases             array of string     YES        Every variant/sub-area/typo spelling that other
                                                   JSONs or docs use for this place, so the reverse-
                                                   indexer can match them. [] if none. This is the
                                                   controlled vocabulary / synonym table.
sources             array of string     YES        "gs1-walkthrough" always; add "gs1-maps" if mapped.
conflicts           array of object     NO         Rarely used (a gazetteer has no multi-source
                                                   numeric disputes). Same shape as elsewhere.
```

The reverse index (which djinn/equipment/monsters/bosses/shops/items tie to each
place) is **NOT stored here** — it is a derived materialized view in a separate
file. See [Derived view: `location_refs.json`](#derived-view-location_refsjson).

### Location Type Enum

| Value | Meaning |
|---|---|
| `town` | Inhabited town / village with shops or NPCs (Vale, Tolbi, Imil). |
| `dungeon` | Cave, forest, desert, lighthouse, fortress — any explorable hazard area. |
| `world_map` | The overworld itself (single `world-map` entry; Djinn met directly on it). |
| `other` | Ship, temple-hub, post-credits peninsula — neither town nor dungeon proper. |

### Example Entry: Mercury Lighthouse

```json
{
  "id": "mercury-lighthouse",
  "name": "Mercury Lighthouse",
  "game": "gs1",
  "type": "dungeon",
  "region": "Angara",
  "chapter_first_seen": "Chapter 3",
  "summary": "The water elemental lighthouse beside Imil; Mia joins the party and Saturos is fought at its aerie.",
  "has_map": true,
  "aliases": ["Mercury Lighthouse Aerie"],
  "sources": ["gs1-walkthrough", "gs1-maps"]
}
```

### Derived view: `location_refs.json`

The "what entities are tied to each place" reverse index is **derived data**, not
authored — so it lives in its own file (`data/gs1/location_refs.json`), generated
by `scripts/locations_refs.py`. It is a materialized view: each entity already
records its own location (`djinn.location.area`, `equipment.acquisition.location`,
`monsters.found[]`, `bosses.encounters[].location`, shop name,
`items.acquisition.location`); the script resolves those (often dirty) strings to
canonical location ids via this schema's `aliases`, then inverts the relation.

Keep `locations.json` (the hand-authored dimension table) and `location_refs.json`
(the generated view) physically separate. **Do not hand-edit `location_refs.json`**
— rebuild it with `python scripts/locations_refs.py`. Shape:

```json
{
  "generated_by": "scripts/locations_refs.py",
  "note": "DERIVED materialized view. Do not hand-edit.",
  "source_files": ["djinn.json", "equipment.json", "monsters.json", "bosses.json", "shops.json", "items.json"],
  "locations": {
    "mercury-lighthouse": {
      "djinn": ["fizz", "sleet"], "equipment": ["..."], "monsters": ["..."],
      "bosses": ["saturos"], "shops": [], "items": ["..."]
    }
  }
}
```

The script also prints a **data-quality report** to stdout: any location string
it could not resolve (→ add an alias here and rerun), plus zero-ref locations.

### Notes for CC (Extraction Instructions)

1. **D1 (skeleton, hand-authored)**: build the canonical list by unioning the
   `### {place}` headers of `gs1_walkthrough.md` with the categorized entries of
   `gs1_maps.md`. Fold sub-areas/revisits into `aliases` (e.g. `Vale Cave` stays
   separate, but `Imil Ice Sliding` / `Imil (Post-Lighthouse)` → aliases of `imil`;
   `Crossbone Isle #1..#10` / `Ghost Ship` → aliases of `crossbone-isle`).
2. **type from gs1_maps.md categories** (Towns & Villages → town, Dungeons & Caves
   → dungeon); `has_map` = true iff that location has a `done` map there.
3. **chapter_first_seen** uses `gs1_walkthrough.md` numbering (NOT the fable
   variant, which renumbers chapters). First arrival, not revisit.
4. **aliases are the matching keys** — seed them from the actual location strings
   in the other JSONs (typos included: `Bibilin Cave`, `Atmiller Cave`,
   `Tolbi-bound Boat`). The richer the aliases, the fewer D2 misses.
5. **Never hand-author the reverse index** — `scripts/locations_refs.py`
   generates `location_refs.json` from the entity files and prints an
   unmatched-string report. To fix a miss, add an alias here and rerun.
6. **No conflict marking** — this is a name registry, not a multi-source numeric
   table. Leave `conflicts` off.
7. **Do not invent summaries** — keep them to facts present in the walkthrough.

---

## Schema: `characters`

One entry per playable party member. GS1 has 4 permanent members (Isaac, Garet,
Ivan, Mia) plus Jenna, who is playable only in the Prologue. This is a small
**dimension table**: it gives the character references scattered across other
files (`equipment.equippable_by`, `psynergy.available_to`,
`classes.available_to[].character`) a real node to resolve against.

File: `data/gs1/characters.json`

```
Field           Type              Required   Notes
---------------------------------------------------------------------------
id              string            YES        Lowercase. e.g. "isaac", "mia".
name            string            YES        In-game display name (default). Matches the
                                             strings used in equippable_by / available_to.
game            string            YES        Always "gs1".
element         string            YES        Innate element: "earth" (Venus) | "fire" (Mars) |
                                             "wind" (Jupiter) | "water" (Mercury).
is_permanent    boolean           YES        true if a permanent GS1 party member; false for
                                             Jenna (Prologue-only).
notes           string | null     NO         Short factual note (role, when they join).
...
```json
{
  ...
  "element": "earth",
  "is_permanent": true,
  "notes": "Venus Adept and the silent protagonist; party leader for all of GS1.",
  "sources": ["fandom-wiki"]
}
```

The character names in `equipment.equippable_by`, `psynergy.available_to`, and
`classes.available_to[].character` are **natural keys** into this table (clean,
unique, 1:1 with `id`). `scripts/links_audit.py` validates that every such
reference matches a `name` here — no separate id field is added at those sites.
